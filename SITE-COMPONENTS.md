...
- `articlePlaylistPreview({id, title, curator, description, anchor, owned})`: a playlist recommendation row with visible curator and editorial note, an explicit full-playlist link and one 152px official Spotify preview. It shows the opening tracks without the large empty area produced by a taller iframe and remains distinct from the 420px full-list presentation used on the homepage. Set `owned: true` for a thecatrave-curated list so the relationship is disclosed in the row.
