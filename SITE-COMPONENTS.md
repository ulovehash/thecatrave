...
- `articlePlaylistPreview({id, title, curator, description, anchor, owned})`: a playlist recommendation row with visible curator and editorial note, an explicit full-playlist link and one compact official Spotify playlist frame. The frame is capped at 300px so it behaves as a short track preview rather than the 420px full-list presentation used on the homepage. Set `owned: true` for a thecatrave-curated list so the relationship is disclosed in the row.
