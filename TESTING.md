...
| Layout & a11y | `npm run check:layout` | Playwright + axe-core | no horizontal overflow, homepage grid reflows to one orphan-free row on wide, Read Next 1/2/4 columns, full-bleed blocks do not clash with a toned section, images carry `width`/`height` (CLS), no uncaught script errors, at 375 / 834 / 1440 px, each page loaded once per width; no serious/critical accessibility violations, at 375 px only |
| Performance & CWV (on demand, not in `npm run check`) | `npm run check:vitals`, or the manual `vitals` workflow in CI | Unlighthouse | site-wide Lighthouse: performance ≥ 0.9, accessibility ≥ 0.95, best-practices ≥ 0.95, SEO = 1, Core Web Vitals, crawlability, mobile-friendliness |
